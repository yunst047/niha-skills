// Vendored from ponytail (github.com/DietrichGebert/ponytail), MIT, so both
// arms pass through the identical correctness gate. © Dietrich Gebert.
//
...
function tmpFile(ext, content) {
  const p = path.join(os.tmpdir(), `koyuki-bench-${Date.now()}-${Math.random().toString(36).slice(2)}${ext}`);
